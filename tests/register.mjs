...
registerHooks({resolve(specifier,context,next) {
  if((specifier === '@/lib/app-user' || specifier === '@/lib/branding-storage') && (context.parentURL?.endsWith('/api/folders/route.ts') || context.parentURL?.endsWith('/api/question-imports/route.ts'))) return next(pathToFileURL(path.resolve('tests/folder-test-auth.mjs')).href,context);

...
### Discarding unfinished student work

Students can use the bin beside unfinished work or in its answer toolbar, then confirm to discard only their progress on that assessment. A teacher-assigned assessment stays available with blank answers; a personal error practice session is removed. Submitted or previously submitted work cannot be discarded. The server checks ownership and revision, uses the same lock ordering as saves, and records an audit event. Migration `017_discard_progress.sql` adds a revision barrier so stale local drafts cannot restore deleted progress. The cloud build applies this migration automatically. `npm run test:discard` covers ownership, submission protection, reset/deletion, stale saves, and local draft recovery.

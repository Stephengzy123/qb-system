...
export function parseLocalDraft(raw:string|null,questions:{id:string;choices:{id:string}[]}[]):LocalDraft|null {
  if(!raw)return null;
  try{
    const value=JSON.parse(raw);
    if(!Number.isSafeInteger(value.revision)||value.revision<0||!value.answers||typeof value.answers!=='object'||Array.isArray(value.answers)||!Number.isFinite(value.cooldownUntil))return null;

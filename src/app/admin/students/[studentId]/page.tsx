...
export default async function StudentProfile({params}:{params:Promise<{studentId:string}>}) {
  ...
  const {studentId}=await params;const students=await listStudents();const student=students.find(s=>s.id===studentId);if(!student)notFound();
  const linked=students.filter(s=>s.duplicate_of_id===studentId);
  return <Workspace view="students" user={user} organizationName={await getOrganizationName()}><p><Link href="/admin/students">← Students</Link></p><header className="page-header"><div><h1>{student.name}</h1><p className="lede">@{student.username} · {student.email}</p><p>{student.completed} completed assignments · Overall accuracy: {accuracy(student.correct,student.graded)} ({student.correct}/{student.graded})</p></div></header>
    <StudentAccountManager key={`${student.id}-${Boolean(student.disabled_at)}`} studentId={student.id} disabled={Boolean(student.disabled_at)} duplicateOf={student.duplicate_of_id} accounts={students.filter(s=>!s.disabled_at&&!s.duplicate_of_id&&s.approval_status==='active')} />
    ...
    <AssignmentHistory items={await studentHistory(studentId)} studentId={studentId} />
  </Workspace>;
}

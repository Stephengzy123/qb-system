...
export default async function StudentPage({ searchParams }: { searchParams: Promise<{ classId?: string }> }) {
  ...
  return <div className="student-shell">
    ...
    <main className="student-main">
      ...
      {!isPreview && <><ErrorPractice />{practices.length>0&&<section className="panel answer-editor"><h2>Continue error practice</h2>{practices.map(item=><Link className="assignment-card-link assignment-history-card" key={item.id} href={`/student/assignments/${item.id}`}>Error practice · {item.total} questions →</Link>)}</section>}</>}

...
export default async function ImportReview({params}:{params:Promise<{importId:string}>}) {
  ...
  return <Workspace view="library" user={user} organizationName={await getOrganizationName()}>
    ...
    <section className="panel question-upload"><h2>{job.succeeded_files} of {job.total_files} images verified in R2</h2><p>Draft set · {job.choice_count} choices per question · {job.status.replaceAll('_',' ')}</p>

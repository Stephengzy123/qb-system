...
export async function GET(_request:Request,{params}:{params:Promise<{assetId:string}>}) {
  ...
  const asset=(await getDatabase().query(`SELECT a.* FROM assets a JOIN import_files f ON f.asset_id=a.id JOIN imports i ON i.id=f.import_id JOIN question_sets s ON s.id=i.set_id WHERE a.id=$1 AND a.status='active' AND ($3::boolean OR i.created_by=$2 OR EXISTS(SELECT 1 FROM folder_permissions p WHERE p.folder_id=s.folder_id AND p.user_id=$2 AND p.can_view))`,[assetId,user.id,user.role==='admin'])).rows[0];

...
export async function PATCH(request: Request, { params }: { params: Promise<{ classId: string; membershipId: string }> }) {
  ...
  const result = await database.query("UPDATE class_memberships SET status = $1, resolved_at = now(), resolved_by = $2 WHERE id = $3 AND class_id = $4 AND role = 'student' AND status = 'pending' RETURNING id", [body.status, user.id, membershipId, classId]);

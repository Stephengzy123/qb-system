...
export default function LoginForm() {
  ...
  return <main className="auth-page"><section className="auth-card"><div className="auth-brand"><div className="mark"><span>Q</span></div><span><strong>AOMA</strong><small>Question Bank</small></span></div><div className="auth-heading"><h1>{mode === "login" ? "Welcome back" : "Create student account"}</h1><p>{mode === "login" ? "Sign in to continue to your workspace." : "After signing up, join a class with its code. A teacher or administrator approves the request."}</p></div><div className="auth-tabs"><button className={mode === "login" ? "active" : ""} onClick={() => setMode("login")}>Sign in</button><button className={mode === "signup" ? "active" : ""} onClick={() => setMode("signup")}>Student sign up</button></div><form onSubmit={submit}>{mode === "signup" && <label><span>Full name</span><input name="name" autoComplete="name" required /></label>}<label><span>Email</span><input name="email" type="email" autoComplete="email" required /></label><label><span>Password</span><input name="password" type="password" minLength={10} autoComplete={mode === "signup" ? "new-password" : "current-password"} required /><small>At least 10 characters</small></label>{error && <p className="auth-error" role="alert">{error}</p>}<button className="primary-button wide" disabled={working}>{working ? "Please wait…" : mode === "login" ? "Sign in" : "Create account"}</button></form></section></main>;
}

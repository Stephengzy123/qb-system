...
export default function ClassDetail({ id, name, class_code, members }: { id: string; name: string; class_code: string | null; members: Member[] }) {
  ...
  return <>
    ...
    <section className="panel member-panel"><div className="panel-heading"><div><h2>Active students</h2><p>{active.length} enrolled</p></div></div>{active.length === 0 ? <div className="compact-empty">No active students yet.</div> : active.map((member) => <div className="member-row" key={member.id}><span className="class-avatar">{member.name.split(/\s+/).map((part) => part[0]).join("").slice(0,2)}</span><span><strong>{member.name}</strong><small>@{member.username ?? "student"}</small></span><b className="status-badge connected">Active</b></div>)}</section>
